...
class Particle {
    ...
    update(timeScale) {
        ...
        const flowAngle =
            (Math.cos(this.x * scale + time) +
                Math.sin(this.y * scale + time)) *
                Math.PI +
            this.angleOffset;

        this.speedX +=
            Math.cos(flowAngle) * 0.1 * this.speedModifier * timeScale;
        this.speedY +=
            Math.sin(flowAngle) * 0.1 * this.speedModifier * timeScale;

        // Friction
        this.speedX *= Math.pow(this.friction, timeScale);
        this.speedY *= Math.pow(this.friction, timeScale);
        ...
        if (mouse.x != undefined && mouse.y != undefined) {
            ...
            if (distance < mouse.radius) {
                ...
                this.speedX -= directionX * timeScale;
                this.speedY -= directionY * timeScale;
            }
        }
...
function animate(timeStamp) {
    ...
    const timeScale = safeDeltaTime / 16.67;

    // Update time for the flow field
    time += 0.002 * timeScale;

    // Adjust trail fade based on timeScale to keep trail length consistent in time
    // Using 0.95 instead of 0.9 makes the trails last longer (more permanent)
    const fadeAlpha = 1 - Math.pow(0.95, timeScale);
    ctx.fillStyle = `rgba(26, 26, 26, ${fadeAlpha})`;
    ...
    for (let i = 0; i < particlesArray.length; i++) {
        particlesArray[i].update(timeScale);
